...
void Online()
{
 ...
 //gSystem->Load("/home/daq/zhl/anaroot/lib/libanacore.so");//load lib of ana root
 ...
 while(estore->GetNextEvent()){
  ...
  for(int i=0;i<rawevent->GetNumSeg();i++){//get the number of segment, usually the types of modules, because the data of same module is sent together
   ...
   for(int j=0;j<seg->GetNumData();j++){//return the number of data_array.size()
   //for(int j=0;j<66;j++){//return the number of data_array.size()
...
//the structure of data_array:geo+channel+value, anaroot will convert it
   ...
   // if(geo == 10) m4->Fill(ch,val);
   // if(geo == 10) m5->Fill(ch,val);
   // if(geo == 10) m6->Fill(ch,val);
   // if(geo == 10) m7->Fill(ch,val);

   if(geo == 20) g1->Fill(ch,val);
   // if(geo == 41) g2->Fill(ch,val);
