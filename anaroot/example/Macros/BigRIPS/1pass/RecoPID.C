...
void RecoPID(){
  ...
  TArtCalibPID *calib_pid= new TArtCalibPID();
  ...
  TClonesArray *fpl_array  = reco_pid->GetFocalPlaneArray();
  ...
  while(estore->GetNextEvent() && neve<10000){
    calib_pid->ReconstructData();
    reco_pid->ReconstructData();

    TArtIC *beam_ic = calib_pid->GetCalibIC()->FindIC(&n);
    ntp_br->Fill(beam_br->GetAoQ(),beam_br->GetZet());
    ntp_zd->Fill(beam_zd->GetAoQ(),beam_zd->GetZet(),beam_zd->GetBeta(),beam_ic->GetEnergySqSum());

    TArtRecoFocalPlane * fplreco = reco_pid->GetRecoFocalPlane();
    for(int i=0;i<12;i++){
      TArtFocalPlane *fpl = fplreco->FindFocalPlane(i);
      if(NULL==fpl) continue;
      TVectorD * optvec = fpl->GetOptVector();
      hFplPos[i]->Fill(optvec(0),optvec(2));
      hFplAngle[i]->Fill(optvec(1),optvec(3));
    }
    for(int i=0;i<rips_array->GetEntries();i++){
  ...
  // histograms for FocalPlane
  TCanvas *cfpl = new TCanvas("cfpl","cfpl",1600,1200);
  cfpl->Divide(4,3);
  for(int i=0;i<fpl_array->GetEntries();i++) {
    cfpl->cd(i+1); hFplPos[i]->Draw("colz");
  }
  if(printGif)cfpl->Print("cfpl.gif");

  TCanvas *cfpla = new TCanvas("cfpla","cfpla",1600,1200);
  cfpla->Divide(4,3);
  for(int i=0;i<fpl_array->GetEntries();i++) {
    cfpla->cd(i+1); hFplAngle[i]->Draw("colz");
  }
  if(printGif)cfpla->Print("cfpla.gif");
